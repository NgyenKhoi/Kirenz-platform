...
export const chatService = {
  ...
  addParticipant: async (conversationId: string, userId: string): Promise<void> => {
    await axios.post(`${API_BASE_URL}/conversations/${conversationId}/participants`, null, {
      params: { userId },
      headers: getAuthHeaders(),
    });
  },
  ...
  removeParticipant: async (conversationId: string, userId: string): Promise<void> => {
    await axios.delete(`${API_BASE_URL}/conversations/${conversationId}/participants/${userId}`, {
      headers: getAuthHeaders(),
    });
  },
