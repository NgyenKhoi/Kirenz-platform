...
export const adminService = {
  ...
  banUser: async (id: string, body: { reason: string; note?: string }) => (await apiClient.post<ApiResponse<AdminUser>>(`/admin/users/${id}/ban`, body)).data.data,
  ...
  suspendUser: async (id: string, body: { suspendedUntil: string; moderationReason: string; note?: string }) => (await apiClient.post<ApiResponse<AdminUser>>(`/admin/users/${id}/suspend`, body)).data.data,
  warnUser: async (id: string, body: { reason: string; message: string; note?: string }) => (await apiClient.post<ApiResponse<AdminAction>>(`/admin/users/${id}/warnings`, body)).data.data,
  ...
  resolveReport: async (id: string, body: { action: string; moderationReason: string; adminNote?: string; suspendedUntil?: string }) => (await apiClient.post<ApiResponse<AdminReport>>(`/admin/reports/${id}/resolve`, body)).data.data,
