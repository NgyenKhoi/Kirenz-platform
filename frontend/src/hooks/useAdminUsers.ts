...
export function useAdminUsers(params: Record<string, unknown>, selectedId?: string) {
  ...
  const ban = useMutation({ mutationFn: ({ id, ...body }: { id: string; reason: string; note?: string }) => adminService.banUser(id, body), onSuccess: invalidate });
  ...
  const suspend = useMutation({ mutationFn: ({ id, ...body }: { id: string; suspendedUntil: string; moderationReason: string; note?: string }) => adminService.suspendUser(id, body), onSuccess: invalidate });
  const warn = useMutation({ mutationFn: ({ id, ...body }: { id: string; reason: string; message: string; note?: string }) => adminService.warnUser(id, body), onSuccess: async () => { await invalidate(); await actions.refetch(); } });
